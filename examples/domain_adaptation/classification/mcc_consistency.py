...
sys.path.append('../../..')
from dalib.adaptation.mcc_consistency import MinimumClassConfusionLoss, ImageClassifier, MCCConsistency
import common.vision.datasets as datasets
import common.vision.models as models
from common.vision.datasets.randaugment import RandAugment
...
def main(args: argparse.Namespace):
    ...
    if args.data_processing == "ours":
        Resize = T.Resize(256) ## resize short size to 256
        Crop = T.RandomCrop(224)
    elif args.data_processing == "dalib":
        Resize = ResizeImage(256)  ## resize to (256, 256)
    ...
    if args.center_crop:
        ...
        train_transform_stong = T.Compose([
            Resize,
            T.CenterCrop(224),
            T.RandomHorizontalFlip(),
            RandAugment(2, 10),
    ...
    train_target_dataset = dataset(root=args.root, task=args.target, download=True, transform=MultipleApply([train_transform, train_transform_stong]))
    ...
    if args.data == 'DomainNet':
        ...
        test_dataset = dataset(root=args.root, task="Test", download=True, transform=val_transform)
        test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)
    else:
    ...
    mcc_consistency_loss = MCCConsistency(temperature=args.temperature, thr=args.thr)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier, mcc_loss, mcc_consistency_loss, optimizer,
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          model: ImageClassifier, mcc: MinimumClassConfusionLoss, consistency, optimizer: SGD,
    ...
    mask_ratio = AverageMeter('selec. ratio', ':3.2f')

    progress = ProgressMeter(
        args.iters_per_epoch,
        [batch_time, data_time, losses, trans_losses, cls_accs, mask_ratio],
    ...
    for i in range(args.iters_per_epoch):
        ...
        (x_t, x_t_strong), _ = next(train_target_iter)
        ...
        loss = cls_loss + transfer_loss * args.trade_off + consistency_loss * args.trade_off_consistency
        ...
        mask_ratio.update(selec_ratio, x_s.size(0))
